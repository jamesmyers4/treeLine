...
export async function interpretPage(pageState: PageState): Promise<PageInterpretation> {
          ...
          interactiveElements: {
            ...
            items: {
              ...
              properties: {
                ...
                purpose: { type: 'string' },
                testIdPresent: { type: 'boolean' }
              },
              required: ['role', 'accessibleName', 'purpose', 'testIdPresent']
            }
          },
          keyDataEntities: { type: 'array', items: { type: 'string' } },
          confidence: { type: 'number' }
        },
        required: ['pageType', 'purpose', 'interactiveElements', 'keyDataEntities', 'confidence']
      }
    }],
  ...
  if (!toolUseBlock || toolUseBlock.type !== 'tool_use') {
    throw new Error(`interpret_page tool_use block missing from API response for URL: ${pageState.url}`)
  }
  const input = toolUseBlock.input as Record<string, unknown>
  if (
    typeof input.pageType !== 'string' ||
    typeof input.purpose !== 'string' ||
    !Array.isArray(input.interactiveElements) ||
    !Array.isArray(input.keyDataEntities) ||
    typeof input.confidence !== 'number'
  ) {
    throw new Error(`interpret_page tool_use input has unexpected shape for URL: ${pageState.url}`)
  }
  return {
    url: pageState.url,
    ...
    interactiveElements: input.interactiveElements as PageInterpretation['interactiveElements'],
